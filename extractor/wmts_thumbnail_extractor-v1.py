...
class WMTSThumbnailExtractor:
    def __init__(self, service_url, output_dir="thumbnails-wmts"):
        ...
        self.scale_reductions = [0, 10, 20, 30]
        
        # Niveaux de zoom WMTS à tester
        self.wmts_zoom_levels = [0, 1, 2, 3, 4, 5, 6, 7, 8]
        
    def get_wmts_layers(self):
        """Récupère les couches disponibles du service WMTS"""
        params = {
            'service': 'WMTS',
            'version': '1.0.0',
    ...
    def extract_wmts_tile(self, layer_name, tile_matrix_set, col, row, zoom, 
    ...
    def extract_wmts_thumbnail_for_zone(self, layer_name, tile_matrix_set, bbox, 
            ...
            for col in range(col_min, col_max + 1):
                for row in range(row_min, row_max + 1):
                    tile_img = self.extract_wmts_tile(
                        layer_name, tile_matrix_set, col, row, zoom_level,
                        verbose=False
                    )
                    if tile_img:
    ...
    def try_france_zoom_levels_wmts(self, layer_name, tile_matrix_set, verbose=True):
        """
            ...
            for scale_reduction in self.scale_reductions:
                ...
                for wmts_zoom in self.wmts_zoom_levels:
                    ...
                    result = self.extract_wmts_thumbnail_for_zone(
                        layer_name, tile_matrix_set, bbox, wmts_zoom,
    ...
    def extract_all_thumbnails(self, verbose=True):
        ...
        print(f"Stratégie: Zoom progressif sur la France (zones + échelles + zooms WMTS)")
        print(f"{'='*70}")
        ...
        for layer in layers:
            ...
            if layer['tile_matrix_sets']:
                ...
                result = self.try_france_zoom_levels_wmts(
                    layer['name'], tile_matrix_set, verbose=verbose
                )
